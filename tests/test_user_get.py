...
class TestUserGet(BaseCase):
    def test_get_details_auth_my_user(self):
        ...
        response3 = requests.post("https://playground.learnqa.ru/api/user/login", data=data1)
        print('данные чужого пользователя -', response3.content)

        # Авторизуюсь под моим пользователем
        response4 = requests.post("https://playground.learnqa.ru/api/user/login", data=data)
        ...
        response5 = requests.get(
            f"https://playground.learnqa.ru/api/user/{user_id_from_auth_method_no_my_user}",
